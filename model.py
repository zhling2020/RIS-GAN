...
class GAN():
    ...
    def generator(self, input_):
        with tf.variable_scope("ORG", reuse=tf.AUTO_REUSE):
            """
            54 Layer Tiramisu
            """
    ...
    def build_model(self):
        ...
        with tf.variable_scope('Generator', reuse=tf.AUTO_REUSE) as scope:
            ...
            self.illum_gt = tf.abs(self.RealA - self.RealB)
            #self.illum_pred = tf.abs(self.RealA - self.FakeB)
            self.Fake_illum = self.generator_i(self.RealA)
            self.Fake_C = self.Fake_illum + self.RealA
            self.i_gt = tf.divide(self.RealA, self.RealB)
            self.Fake_i = self.generator_i(self.RealA)
            self.Fake_Ci = tf.divide(self.RealA, self.Fake_i)

        with tf.name_scope('Real_Discriminator'):
            with tf.variable_scope('Discriminator') as scope:
                self.predict_real = self.discriminator(self.RealA, self.RealB)
                #self.predict_realillum = self.discriminator(self.illum_pred, self.illum_gt)

        with tf.name_scope('Fake_Discriminator'):
            with tf.variable_scope('Discriminator', reuse=True) as scope:
                self.predict_fake = self.discriminator(self.RealA, self.FakeB)
                #self.predict_fakeillum = self.discriminator(self.illum_gt, self.illum_pred)
        ...
        with tf.name_scope('GeneratorLoss'):
            ...
            self.l1_e_loss = tf.reduce_mean(tf.abs(self.RealB - self.Fake_C))
            self.e_loss = tf.reduce_mean(tf.abs(self.illum_gt - self.Fake_illum))
            self.l1_e_loss1 = tf.reduce_mean(tf.abs(self.RealB - self.Fake_Ci))
            self.e_loss1 = tf.reduce_mean(tf.abs(self.i_gt - self.Fake_i))
            #self.l1_loss = tf.reduce_mean(tf.abs(self.RealB - self.FakeB))
            self.vgg_loss = (1e-5) * tf.losses.mean_squared_error(self.RealB_VGG, self.FakeB_VGG)
            #self.illum_gt = tf.abs(self.RealA - self.RealB)
            #self.illum_pred = tf.abs(self.RealA - self.FakeB)
            
            #self.loss_G_Ang = tf.reduce_mean(tf.abs(self.illum_gt - self.illum_pred))
            #print(self.loss_G_Ang.shape)
            #assert False, "Pause"

            self.G_loss = self.l1_e_loss * 1 + self.e_loss * 10 + self.l1_e_loss1 * 0.2 + self.e_loss1 * 1 + self.gan_wt * self.gan_loss + self.l1_wt * self.l1_loss + self.vgg_wt * self.vgg_loss 
        ...
        with tf.name_scope('Summary'):
            ...
            self.G_summary = tf.summary.merge([gan_loss_sum, l1_loss_sum, l1_e_loss_sum, e_loss_sum, vgg_loss_sum, G_loss_sum])
    ...
    def train(self):
        ...
        if self.custom_data:
            ...
            for i, file in enumerate(train_data):
                self.A_train[i] = cv2.resize(cv2.imread(os.path.join(os.getcwd(), self.A_dir, file), 1).astype(np.float32),(256,256))
                self.B_train[i] = cv2.resize(cv2.imread(os.path.join(os.getcwd(), self.B_dir, file), 1).astype(np.float32),(256,256))

            for i, file in enumerate(val_data):
                self.A_val[i] = cv2.resize(cv2.imread(os.path.join(os.getcwd(), self.A_dir, file), 1).astype(np.float32),(256,256))
                self.B_val[i] = cv2.resize(cv2.imread(os.path.join(os.getcwd(), self.B_dir, file), 1).astype(np.float32),(256,256))
        ...
        with tf.Session() as self.sess:
            ...
            loss_operations = [self.D_loss, self.G_loss, self.gan_loss, self.l1_loss, self.l1_e_loss, self.e_loss, self.vgg_loss]
            ...
            for i in range(start, self.epochs):
                ...
                for j in range(batches):
                    ...
                    if j % logger_frequency == 0:
                        D_loss, G_loss, GAN_loss, L1_loss, l1_e_loss, e_loss, VGG_loss = self.sess.run(loss_operations, feed_dict=feed_dict)
                        ...
                        line = 'Batch: %d, G_Loss: %.3f, GAN: %.3f, L1: %.3f, L1e: %.3f, e: %.3f, P: %.3f' % (
                            j, G_loss, GAN_loss, L1_loss, l1_e_loss, e_loss, VGG_loss)
                        print(line)
                        print(self.RealA)
                        print('B')
                        print(self.FakeB)
                        print('C')
                        print(self.Fake_illum)
